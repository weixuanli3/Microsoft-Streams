...
def test_register_password_incorrect_length():
    with pytest.raises(InputError):
        auth_register_v1("john.doe@unsw.edu.au","123","John","Doe")
...
def test_register_first_name_incorrect_length():
    with pytest.raises(InputError):
        auth_register_v1("john.doe@unsw.edu.au","password","","Doe")

    with pytest.raises(InputError):
        auth_register_v1("john.doe@unsw.edu.au","password","JohnJohnJohnJohnJohnJohnJohnJohnJohnJohnJohnJohnJohn","Doe")

# Last name must be between 1 and 50 characters inclusive
def test_register_last_name_incorrect_length():
    with pytest.raises(InputError):
        auth_register_v1("john.doe@unsw.edu.au","password","John","")

    with pytest.raises(InputError):
        auth_register_v1("john.doe@unsw.edu.au","password","John","DoeDoeDoeDoeDoeDoeDoeDoeDoeDoeDoeDoeDoeDoeDoeDoeDoe")
