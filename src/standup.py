...
def standup_start_v1(token, channel_id, length):
    ...
    if curr_channel['standup']['finish_time'] < datetime.datetime.now().replace(tzinfo=timezone.utc).timestamp():
        curr_channel['standup']['finish_time'] = datetime.datetime.now().replace(tzinfo=timezone.utc).timestamp() + length
